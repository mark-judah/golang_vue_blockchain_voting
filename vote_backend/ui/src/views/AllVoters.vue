<template>
    <div class="">
      ...
      <div class="flex flex-row justify-between items-center w-full">
        <div>
          <p class="px-12">Voters</p>
        </div>

        <div class="flex flex-row px-12">
          <router-link :to="{ name: 'voters' }" @click="$router.go()" class="flex items-center p-2">
            <img src="../assets/images/refresh.svg" class="w-6 h-8">
          </router-link>

          <router-link :to="{ name: 'new-voter' }" class="flex items-center p-2">
                ...
                <table class="table-auto space-y-6">
                    <thead class="">
                        ...
                        <tr class="rounded-3xl">
                            ...
                            <th class="px-1.5 py-2 border">First Name</th>
                            <th class="px-1.5 py-2 border">Last Name</th>
                            <th class="px-1.5 py-2 border">ID Number</th>
                            <th class="px-1.5 py-2 border">Phone Number</th>
                    ...
                    <tr v-for="(data, i) in this.allVoters"  :key="data" class="bg-white border-b">
                            <td class="px-1.5 py-2   border ">{{++i }}</td>
                            <td class="px-1.5 py-2   border "  >{{ data.FirstName   }}</td>
                            <td class="px-1.5 py-2   border "  >{{ data.LastName   }}</td>
                            <td class="px-1.5 py-2   border "  >{{ data.IDNumber   }}</td>
                            <td class="px-1.5 py-2   border "  >{{ data.PhoneNumber   }}</td>
...
export default {
  ...
  methods: {
    getVoters () {
      ...
      axios.get(
        'http://127.0.0.1:3500/api/secured/get-all-voters', config
      ).then((response) => {
        console.log(response.data)
        this.allVoters = response.data
