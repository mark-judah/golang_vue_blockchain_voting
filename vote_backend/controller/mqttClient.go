...
var receiveMsgs mqtt.MessageHandler = func(client mqtt.Client, message mqtt.Message) {
	...
	if err != nil {
		...
		if err != nil {
			...
			if err != nil {
				fmt.Println("Error unmarshalling as string,  string array or as Queue:", err)
			}
		}
	...
	if string(message.Topic()) == "raftLogAppend/1" {
		...
		if utils.GetClientState() == "follower" {
			Log = raftLog
			jsonData, err2 := json.Marshal("log replicated to follower node")
			if err2 != nil {
				panic(err2)
			}
	...
	if string(message.Topic()) == "raftLogAppendConfirmation/1" {
		...
		// if utils.GetClientState() == "leader" {
		//if majority of follower nodes have updated their log, the nodes can update their databases
		...
		if logAppendConfirmations >= getTotalConnectedNodes()/2 {
			...
			values := reflect.ValueOf(Log.Transactions)
			fmt.Println(values)

			//loop through array of structs to get each struct
			for i, x := range Log.Transactions {
				fmt.Println(i)
				fmt.Println(x)

				//extract values from the structs
				structs := reflect.ValueOf(x)
				types := structs.Type()
				fmt.Println(types)
				...
				transaction := models.Transaction{Txid: x.Txid, NodeId: x.NodeId, CandidateId: x.CandidateId, TransactionHash: x.TransactionHash}
