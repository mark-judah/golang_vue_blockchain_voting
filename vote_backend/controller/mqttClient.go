...
var receiveMsgs mqtt.MessageHandler = func(client mqtt.Client, message mqtt.Message) {
	...
	if string(message.Topic()) == "raftLogAppendConfirmation/1" {
		...
		if utils.GetClientState() == "leader" {
			//if majority of follower nodes have updated their log, the leader can update its database
			...
			if logAppendConfirmations >= getTotalConnectedNodes()/2 {
				...
				statement, err2 := database.Prepare(
					"INSERT INTO transactions (transaction_id,desktop_id,candidate_id,hash,created_by) VALUES(?, ?, ?, ?, ?)")
				if err2 != nil {
					panic(err2)
				}
				statement.Exec("test2", "test", "test", "test", "test")
			}
